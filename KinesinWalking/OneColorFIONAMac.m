function varargout = OneColorFIONAMac(varargin)
% ONECOLORFIONAMAC MATLAB code for OneColorFIONAMac.fig
%      ONECOLORFIONAMAC, by itself, creates a new ONECOLORFIONAMAC or raises the existing
%      singleton*.
%
%      H = ONECOLORFIONAMAC returns the handle to a new ONECOLORFIONAMAC or the handle to
%      the existing singleton*.
%
%      ONECOLORFIONAMAC('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ONECOLORFIONAMAC.M with the given input arguments.
%
%      ONECOLORFIONAMAC('Property','Value',...) creates a new ONECOLORFIONAMAC or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before OneColorFIONAMac_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to OneColorFIONAMac_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help OneColorFIONAMac

% Last Modified by GUIDE v2.5 13-Apr-2016 11:06:25

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @OneColorFIONAMac_OpeningFcn, ...
                   'gui_OutputFcn',  @OneColorFIONAMac_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before OneColorFIONAMac is made visible.
function OneColorFIONAMac_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to OneColorFIONAMac (see VARARGIN)
% UIWAIT makes OneColorFIONAMac wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% Choose default command line output for OneColorFIONAMac
...
function varargout = OneColorFIONAMac_OutputFcn(hObject, eventdata, handles) 
...
if length(Cursor)==2
    ...
    elseif OpenTransformedFile == 1
        DataPath = strrep(DataPath,'\transformed','');
...
if OpenTransformedFile; DataPath = [DataPath '\transformed']; end
cd(DataPath);
